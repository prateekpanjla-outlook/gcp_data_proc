#!/bin/bash
# Deploy Test Infrastructure using Terraform Deployer Service Account
# Usage: ./terraform_deploy_test.sh [PROJECT_ID] [ENVIRONMENT] [REGION]
#
# Example:
#   ./terraform_deploy_test.sh my-test-project-12345 dev us-central1
...
# =============================================================================
LOG_DIR="logs"
LOG_FILE="${LOG_DIR}/terraform-test-deploy-$(date +%Y%m%d-%H%M%S).log"
...
if [[ -z "$PROJECT_ID" ]]; then
  ...
  echo "   or: PROJECT_ID=your-test-project-id $0"
  exit 1
fi
...
TERRAFORM_DIR="${SCRIPT_DIR}/../terraform-test"  # Changed to terraform-test
SA_ID="test-terraform-deployer"  # Fixed to test environment
SA_EMAIL="${SA_ID}@${PROJECT_ID}.iam.gserviceaccount.com"
KEY_FILE="${SCRIPT_DIR}/../secrets/terraform/${SA_ID}-${PROJECT_ID}.json"

echo "========================================="
echo "Test Terraform Deployment"
...
if [[ ! -f "${KEY_FILE}" ]]; then
  ...
  echo "  Run: ./create-terraform-deployer-account.sh ${PROJECT_ID} test"
  exit 1
fi
...
echo "  Command: terraform plan -var-file=\"terraform.tfvars\""
echo ""

if terraform plan \
  -var-file="terraform.tfvars"; then
  echo ""
